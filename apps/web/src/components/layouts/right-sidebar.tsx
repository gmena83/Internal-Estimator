...
import { Card, Badge, Sparkline } from "@/components/ui/UIComponents";
      ...
      <div className="p-4 border-b border-border">
        ...
        <div className="space-y-3">
          {health.map((h) => (
            <div
              key={h.provider}
              className="flex flex-col gap-2 p-3 rounded bg-card border border-border"
            >
              {/* Header */}
                  ...
                  <div className="flex items-center justify-between">
                    <span
                      className={cn(
                        ...
                        h.errorRate > 0 ? "text-red-400" : "text-muted-foreground",
                      )}
                    >
                      {h.errorRate}%
                    </span>
              ...
              <div className="w-full bg-background h-1 rounded-full overflow-hidden opacity-50">
                <div
                  ...
                  style={
                    {
                      "--progress": `${Math.min(100, (h.latency / 1000) * 100)}%`,
        ...
        <div className="mt-4 p-3 rounded bg-card border border-border">
          <div className="flex items-center justify-between text-xs font-medium mb-2">
            ...
            <span className="text-foreground">
              {usage.storage.toFixed(1)}MB <span className="text-muted-foreground">/ 1GB</span>
            </span>
          </div>
          <div className="w-full bg-background h-2 rounded-full overflow-hidden">
            <div
              className="h-full bg-primary transition-all duration-500 shadow-[0_0_10px_var(--primary-glow)] w-[var(--progress)]"
              style={
                {
                  "--progress": `${Math.min(100, (usage.storage / 1000) * 100)}%`,
                } as React.CSSProperties
              }
            />
          </div>
