...
async function runDecathlon() {
  ...
  for (const scenario of scenarios) {
    ...
    console.log(
      `${status} - ${steps}/5 steps, PDF: ${result.pdfGenerated}, Keywords: ${result.keywordsFound.length}/${scenario.expectedKeywords.length}`,
    );
  ...
  for (const result of results) {
    ...
    console.log(
      `  Steps: ${steps}/5, PDF: ${result.pdfGenerated}, Keywords: ${result.keywordsFound.join(", ") || "none"}`,
    );
  ...
  fs.writeFileSync("decathlon_results.json", JSON.stringify(results, null, 2));
