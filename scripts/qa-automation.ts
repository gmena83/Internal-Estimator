...
async function runSimulation(scenarioIdx: number, scenarioText: string): Promise<QAMetrics> {
  ...
  try {
    ...
    const createRes = await fetch(`${BASE_URL}/api/projects`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-qa-bypass": "true" },
      body: JSON.stringify({
    ...
    if (!createRes.ok)
      throw new Error(`Stage 1 Failed: ${createRes.status} ${createRes.statusText}`);
    ...
    fs.appendFileSync(
      path.join(QA_RESULTS_DIR, "error_log.txt"),
...
function generateReport(results: QAMetrics[]) {
  ...
  if (fs.existsSync(routeLogsPath)) {
    ...
    routeCoverage =
      `Unique Routes Hit: ${uniqueRoutes.size}\n` + Array.from(uniqueRoutes).join("\n");
  }
