...
// );
const hasAnthropicIntegration = false;
...
console.log(
  `AI Integrations: Gemini=${hasGeminiIntegration}, Anthropic=${hasAnthropicIntegration}, OpenAI=${hasOpenAIIntegration}`,
);
...
Be helpful, professional, and concise. Guide users through the estimate and proposal workflow.`,
          ...
          return {
            content:
              response.choices[0]?.message?.content || "I understand. How can I help you further?",
...
[... detailed instructions matching original prompt ...]
...
Return as JSON:
{
  ...
  "guideB": "# Manual B: No-Code Approach\\n\\n## Executive Summary\\n..."
        ...
        try {
          ...
          console.error("Failed to parse OpenAI response for guides:", parseError);
          return generateDefaultVibeGuides(project, scenarioA, scenarioB);
        }
      } catch (aiError) {
        await trackHealth("openai", startTime, aiError as Error);
        console.error("OpenAI error, using fallback guides:", aiError);
      }
    }
  ...
  return {
    scenarioA: {
      ...
      featureBreakdown: [
        {
          ...
          hours: 40,
          cost: 6000,
          expectedRangeLow: 5000,
          expectedRangeHigh: 15000,
        },
        {
          feature: "Backend API Development",
          hours: 80,
          cost: 12000,
          expectedRangeLow: 10000,
          expectedRangeHigh: 25000,
        },
        {
          feature: "Frontend Development",
          hours: 100,
          cost: 15000,
          expectedRangeLow: 12000,
          expectedRangeHigh: 30000,
        },
        {
          feature: "Database Design & Integration",
          hours: 40,
          cost: 6000,
          expectedRangeLow: 5000,
          expectedRangeHigh: 15000,
        },
        {
          ...
          hours: 40,
          cost: 6000,
          expectedRangeLow: 3000,
          expectedRangeHigh: 8000,
        },
      ],
      ...
      ongoingCosts: {
        ...
        annualMaintenanceHigh: Math.round(scenarioACost * 0.4),
      ...
      regionalAlternatives: [
        {
          region: "Latin America (Nearshore)",
          multiplier: 0.45,
          adjustedCost: Math.round(scenarioACost * 0.45),
          notes: "Quality nearshore development",
        },
        {
          region: "Eastern Europe",
          multiplier: 0.4,
          adjustedCost: Math.round(scenarioACost * 0.4),
          notes: "Strong technical talent pool",
        },
        {
          region: "India/Asia",
          multiplier: 0.3,
          adjustedCost: Math.round(scenarioACost * 0.3),
          notes: "Cost-optimized offshore",
        },
      ],
      multipliers: {
    ...
    scenarioB: {
      ...
      featureBreakdown: [
        {
          ...
          hours: 30,
          cost: 2250,
          expectedRangeLow: 2000,
          expectedRangeHigh: 5000,
        },
        {
        ...
        },
        {
          ...
          hours: 40,
          cost: 3000,
          expectedRangeLow: 2500,
          expectedRangeHigh: 6000,
        },
        {
          feature: "Testing & Iteration",
          hours: 20,
          cost: 1500,
          expectedRangeLow: 1000,
          expectedRangeHigh: 3000,
        },
      ],
      techStack: ["Airtable", "Zapier", "Webflow", "Make"],
      timeline: "3-4 weeks",
      ...
      ongoingCosts: {
        ...
        annualMaintenanceHigh: Math.round(scenarioBCost * 0.4),
      ...
      regionalAlternatives: [
        {
          ...
          multiplier: 0.6,
          adjustedCost: Math.round(scenarioBCost * 0.6),
          notes: "No-code specialists",
        },
        {
          region: "Eastern Europe",
          multiplier: 0.55,
          adjustedCost: Math.round(scenarioBCost * 0.55),
          notes: "Technical automation experts",
        },
      ],
      multipliers: {
    ...
    roiAnalysis: {
      ...
      methodology:
        "Cost of Doing Nothing based on: 1) Manual operational labor ($40K/yr), 2) Lost efficiency opportunities ($10K/yr). Savings assume 70% automation of manual tasks.",
    },
  };
}

function generateEstimateMarkdown(
  project: Project,
  scenarios: any,
  ...
  const formatNum = (val: any, fallback: number = 0): string => {
    const num =
      typeof val === "number" ? val : typeof val === "string" ? parseFloat(val) : fallback;
  ...
  return {
    ...
    objectives:
      objectives.length > 0
        ...
        : ["Define requirements", "Build solution", "Deploy and iterate"],
    constraints: [],
    clientName,
...
function generateDefaultVibeGuides(
  project: Project,
  scenarioA: Scenario | null,
  scenarioB: Scenario | null,
): { guideA: string; guideB: string } {
  const parsedData = (project.parsedData as any) || {};
