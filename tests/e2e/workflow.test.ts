...
describe('E2E Workflow Tests', () => {
  ...
  describe('Error Recovery', () => {
    ...
    it('should validate data at each stage transition', async () => {
      ...
      // Valid transition
      expect(() => validateStageTransition(1, 2, { 
        ...project, 
        estimateMarkdown: 'test' 
      })).not.toThrow();
      
      // Invalid: missing estimate
      expect(() => validateStageTransition(1, 2, project))
        .toThrow('Cannot advance to stage 2 without estimate');
      
      // Invalid: skipping stages
      expect(() => validateStageTransition(1, 3, project))
        .toThrow('Cannot skip from stage 1 to 3');
    });
  });
